...
test('matriz de formatos: pdf, texto e imagem entram no pipeline esperado', () => {
  const cases = [
...
test('matriz de formatos: docx, planilha e vídeo continuam fora do pipeline universal atual', () => {
  const cases = [
    fakeFile({ name: 'treino.docx', type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }),
    fakeFile({ name: 'treino.xlsx', type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
    fakeFile({ name: 'video.mp4', type: 'video/mp4' }),
  ];
...
test('leitores simples identificam imagem por mime type', () => {
