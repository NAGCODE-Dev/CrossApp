...
- `done` Tirar `src/ui/actions.js` do papel de roteador absoluto do atleta
  ...
  - `done` extrair a composição do click context de `apps/athlete/features/actions/setup.js` para `apps/athlete/features/actions/setupClickBindings.js`
...
- `done` Tirar `src/ui/render.js` e `src/ui/actions.js` do papel de dono do WOD/importação do atleta
  ...
  - `done` separar bindings de importação PDF e mídia de `apps/athlete/features/events/importBindings.js` para módulos dedicados
  ...
  - `done` separar navegação do WOD e mudanças de settings/dia de `apps/athlete/actions/todayUiActions.js` para módulos dedicados
...
- `in_progress` Converter render manual/string para componentes por fatias
  ...
  - `done` extrair preparação de view state de `apps/athlete/features/account/page.js` e `apps/athlete/features/history/page.js` para módulos dedicados
  - `done` extrair seções do modal de configurações de `apps/athlete/modals/settingsModal.js` para `apps/athlete/modals/settingsSections.js`
